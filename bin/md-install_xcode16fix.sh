...
# 用户可能存在的profile文件（优先 zshrc）
export bashProfileFiles=(
  "$userHome/.zshrc"
  "$userHome/.zprofile"
...
function downloadFile() { # args: sourceUrl, targetPath
  ...
  curlPath="$(command -v curl)" || panic $? "Failed to get curl path"

  "$curlPath" --output "$targetPath" "$sourceUrl" || panic $? "Failed to download $sourceUrl to $targetPath"
}
...
function downloadGithubTarball() { # args: url, outputDir, title
  # 修复原脚本里 $setcmd 拼写错误：这里不需要再 set，一开始已 set -eo pipefail
  local url="$1"
  local outputDir="$2"
  local title="$3"
  local tempDirForTar
  ...
  untardDir="$(find "$tempDirForFiles/"* -type d -depth 0 -print -quit)" || panic $? "Failed to get untar'ed directory name"
  [[ -n "$untardDir" ]] || panic 1 "Failed to locate untar'ed directory"
...
function changeMode() {
  ...
  if [[ -e "$target" ]]; then
    chmod $options "$mode" "$target" || panic $? "Failed to change mode to $mode on $target"
  fi
}

# 获取用户profile文件（修复：没找到就创建 bashProfileFiles[0]）
...
# find 一个文件名：用 -print -quit 避免 pipefail/head 的坑  [oai_citation:2‡Unix & Linux Stack Exchange](https://unix.stackexchange.com/questions/774908/sigpipe-and-bash-pipefail?utm_source=chatgpt.com)
function find_first() {
  local base="$1"
  local name="$2"
  find "$base" -name "$name" -print -quit 2>/dev/null || true
}

# start it
mkdir -p "$MonkeyDevPath" || panic $? "Failed to make directory: $MonkeyDevPath"
...
# 支持 iphoneos command line tools
iosSdkPlatformPath="$(getSdkProperty iphoneos PlatformPath)"
macosSdkPlatformPath="$(getSdkProperty macosx PlatformPath)"

# --- Xcode16: Embedded-Device.xcspec 选择逻辑（你机器上两条都存在） ---
swiftbuild_spec="/Applications/Xcode.app/Contents/SharedFrameworks/SwiftBuild.framework/Versions/A/PlugIns/SWBBuildService.bundle/Contents/PlugIns/SWBApplePlatformPlugin.bundle/Contents/Frameworks/SWBApplePlatform.framework/Versions/A/Resources/Embedded-Device.xcspec"
xcbuild_spec="/Applications/Xcode.app/Contents/SharedFrameworks/XCBuild.framework/Versions/A/PlugIns/XCBBuildService.bundle/Contents/PlugIns/XCBSpecifications.ideplugin/Contents/Resources/Embedded-Device.xcspec"

specificationFile="$(pick_first_existing "$swiftbuild_spec" "$xcbuild_spec")"
if [[ -z "$specificationFile" ]]; then
  specificationFile="$(find_first /Applications/Xcode.app "Embedded-Device.xcspec")"
fi
...
# --- Xcode16: MacOSX Package/Product Types xcspec ---
xcbuild_pkg="/Applications/Xcode.app/Contents/SharedFrameworks/XCBuild.framework/Versions/A/PlugIns/XCBBuildService.bundle/Contents/PlugIns/XCBSpecifications.ideplugin/Contents/Resources/MacOSX Package Types.xcspec"
xcbuild_prod="/Applications/Xcode.app/Contents/SharedFrameworks/XCBuild.framework/Versions/A/PlugIns/XCBBuildService.bundle/Contents/PlugIns/XCBSpecifications.ideplugin/Contents/Resources/MacOSX Product Types.xcspec"

packageTypesForMacOSXPath="$(pick_first_existing "$xcbuild_pkg")"
productTypesForMacOSXPath="$(pick_first_existing "$xcbuild_prod")"

if [[ -z "$packageTypesForMacOSXPath" ]]; then
  packageTypesForMacOSXPath="$(find_first /Applications/Xcode.app "MacOSX Package Types.xcspec")"
fi
if [[ -z "$productTypesForMacOSXPath" ]]; then
  productTypesForMacOSXPath="$(find_first /Applications/Xcode.app "MacOSX Product Types.xcspec")"
fi
